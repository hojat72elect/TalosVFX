...
public class Tween<T> implements Pool.Poolable {
    ...
    @Getter(AccessLevel.PACKAGE)
    private final Array<Tween<?>> tweensToRunAfter = new Array<>();

    private final TweenData workingData = new TweenData();
    private final TweenData startData = new TweenData();
    @Getter
    private final TweenData targetData = new TweenData();

    @Getter
    private TweenMask<T> mask;
    private int attributesToTween;

    private final TweenSystem tweenManager;
    ...
    private void maskAttributeCountCheck(int suppliedAttributes) {
        if (suppliedAttributes != mask.cachedAttributes().length)
