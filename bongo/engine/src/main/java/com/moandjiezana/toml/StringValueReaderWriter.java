...
class StringValueReaderWriter implements ValueReader, ValueWriter {
    ...
    String replaceUnicodeCharacters(String value) {
        ...
        while (unicodeMatcher.find()) {
            value = value.replace(unicodeMatcher.group(0), new String(Character.toChars(Integer.parseInt(unicodeMatcher.group(1), 16))));
        }
        return value;
    }
    ...
    public boolean canWrite(Object value) {
        return value instanceof String || value instanceof Character || value instanceof Enum;
    }
