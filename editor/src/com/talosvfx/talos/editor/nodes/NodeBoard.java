...
public class NodeBoard<T extends DynamicNodeStageData> extends WidgetGroup implements Observer, EventContextProvider<DynamicNodeStage<?>> {
    ...
    private final Vector2[] curvePoints = new Vector2[4];
    private ObjectIntMap<Class<? extends NodeWidget>> nodeCounter = new ObjectIntMap<>();
    private final ObjectSet<NodeWidget> selectedNodes = new ObjectSet<>();
    private final DynamicNodeStage nodeStage;
    ...
    private final ObjectMap<Integer, NodeWidget> nodeMap = new ObjectMap<>();
    ...
    public void act(float delta) {
        ...
        nodeStage.shouldAutoMove = activeCurve != null || isHoldingNode;
    }
...
//        TalosMain.Instance().ProjectController().setDirty();
    ...
    public void updateSelectionBackgrounds() {
        ...
        for (NodeWidget wrapper : nodeStage.data.nodes) {
            wrapper.setSelected(getSelectedNodes().contains(wrapper));
        }
    }
